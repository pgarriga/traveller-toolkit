...
[![Version](https://img.shields.io/badge/Version-4.0.0-blue?style=flat-square)](https://github.com/pgarriga/traveller-toolkit)
...
|-------|------|--------------|
| `/` | **Home** | Lists the available tools as cards, in three blocks: Navigation, Traffic and Ships. The Ships block lists your fleet — one card per ship, with its name and type — plus a card that creates a new one. |
...
| `/ship` | **My Ship** | Create a ship — name it and pick its type — and edit its sheet, laid out like a stat block in the rulebook: strip out or bolt on weapons, staterooms, craft and systems. Any of the 24 "Common Spacecraft" designs can be its type — see [My Ship](#my-ship) below. |
...
### My Ship
- **Create the ship first** — A small form asks for two things: a name and a type. The type is one of the 24 rulebook designs, which fills the whole sheet in, or *Custom*, which starts it blank. That is the only place the type is chosen: from then on the sheet shows it locked and lets you edit everything else. Want another type? Create another ship.
- **A fleet, not a single ship** — Every ship you create is kept. The home page's Ships block lists them all with their name and type and creates new ones, and opening one makes it the **active** ship — the one the Freight and Passenger calculators read. `/ship` itself is one ship's sheet, so it does not list the others.
- **Export and delete** — Two small actions on the Profile tab, just above the notes: **Export ship** downloads the whole thing as a `.json` file (a copy to keep, or to carry to another browser), and **Delete ship** removes it from the fleet. Deleting the last one drops you back to the create screen.
- **Four tabs** — *Profile* (identity and notes, then engines, power and cargo at a glance), *Details* (the component sheet), *Crew* and *Cargo hold*.
...
- **24 designs from the rulebook** to pick a type from — 14 starships (Scout/Courier, Seeker, Free Trader, Far Trader, Safari Ship, System Defence Boat, Yacht, Gazelle-class Close Escort, Laboratory Ship, Patrol Corvette, Subsidised Merchant, Donosev-class Survey Scout, Subsidised Liner, Mercenary Cruiser) and 10 small craft (light fighter, launch, shuttle, ship's boat, slow boat, pinnace, slow pinnace, modular cutter, ferry, passenger shuttle). The design fills the sheet in as the ship is created; the passenger berths stay yours, since the rulebook does not split staterooms between passage classes.
- **Fit turrets from the rulebook tables** — The weaponry card has no generic "add" menu: it has **Fit a turret** and **Fit a barbette**, each opening a dialog on its own table. Choose the mount (fixed, single, double or triple, each turret also listed in its pop-up version) and fill each of its slots with any of the five turret weapons — mixed loadouts included. Barbettes are in the same list, with the eight barbette weapons of their own table. The tonnage and the Power points are worked out from the two tables, and the sheet then shows the turret on its own line with each weapon listed under it. That line is not typed by hand: click the turret to reopen the dialog and change what it carries.
- **Add and remove components** — Each section has its own "add" menu (except hull, armour, the drives, the power plant, fuel, bridge, computer and sensors, of which a ship carries one set: those are edited rather than added to), built from every part those 24 designs actually carry: turrets from single-empty to triple pulse laser, particle barbettes, fixed mounts, missile magazines, drones, labs, workshops, staterooms, low berths and the rest. "Custom" adds a blank row for anything the rulebook does not list. Accommodation lines carry a **quantity**, since staterooms (4 t) and low berths (0.5 t) are identical repeated pieces: type how many and the line's tonnage follows, and the tonnage stays editable if your ship disagrees with the table. Deleting is behind a **Remove rows** toggle, so the bins stay out of the way — and out of the layout — until you want them.
- **Feeds the calculators** — The cargo bay and the passenger berths live on the sheet, and the Freight and Passenger calculators read them from there. Each is typed into the calculator that uses it and that edits the ship, so there is only ever one answer to "how big is my hold?"; the Profile summary shows both back to you.
- **The cargo hold, as a manifest** — A tab of its own: how much hold the ship has, how much of it is taken and how much is left, and under that a table you fill with what you are actually carrying. Go over the hold and the free figure turns amber and negative — the sheet tells you, it does not stop you.
- **Export and import** — A ship leaves as a `.json` from its Profile tab and comes back from the fleet block on the home page. The file is the sheet itself, so it is also how you hand your ship to another player.
- **Two crews, kept apart** — The Crew tab holds both the crew the *design* calls for (the rulebook's line, filled in by the template) and a roster of who is actually aboard: a full-width table of name, post and monthly salary per person, added from a menu of the twelve shipboard roles, which stacks into cards on a phone. Flying short-handed is a normal state, and the sheet can say so.
- **Power requirements**, worked out for you from the rulebook's formulas: 20% of the hull for basic systems, 10% of the hull per point of Thrust and per jump number, plus the sensor grade and each weapon's own draw. Pick the Thrust, jump, sensor grade and power plant type from those sections' own selectors — and see, beside the total, what the plant actually produces.
- **Totals**, informational only: the components' tonnage against the hull.
- **Hull tonnage is the ship's tonnage** — it heads the hull card instead of sitting in a row, so the components are measured against it rather than added to it.
...
│   ├── ShipCreateModal.tsx # New-ship form: name + type
